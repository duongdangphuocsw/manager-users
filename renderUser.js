...
function getUsersHandle(data) {
  ...
  html = data.map(function (user, index) {
    ...
    return `
        ...
        id="userId_${user.id}"
        >
            <td class="indexUser">${(currentPage -1)* 10 + index + 1}</td>
            <td class="nameUser">${user.name}</td>
            <td class="emailUser">${user.email}</td>
            <td class="groupUser">${user.group_role}</td>
            <td class="activeId_${
              user.id
            } ${isActiveToSetName} statusUser">${activeContent}</td>
            <td class="control">
            <i class="fa-solid fa-pen editBtn"
            onclick="handleEditUser(${user.id})"
            ></i>
            <i class="fa-solid fa-trash deleteBtn" onclick="deleteHandle(${
              user.id
            })"></i>
            <i class="fa-solid fa-user-check activeBtn" onclick="activeUser(${
              user.id
            })"></i></td>
        </tr>
    `;
...
export { dataUsers, getUsersHandle, getData };
